...
with tab1:
    ...
    view = st.checkbox('View all collections')
    if view:
        show_table = st.radio("Select the collection to view: ",("channels","videos","comments"))
...
with tab2:
    ...
    view = st.checkbox('View all tables')
    if view:
        view_table = st.radio("Select the table to view: ",("channels","videos","comments"))
        ...
        if view_table == "channels":
            ...
            myconnection.commit()
            r1=cur.fetchall()
            st.write(pd.DataFrame(r1, columns=['Channel_Name','Channel_Id','Subscribers','Views','Total_Videos',
                                          'Channel_Description',
                                          'Playlist_Id']))

        elif view_table =="videos":
            q2= 'Select * from videos'
            cur.execute(q2)
            myconnection.commit()
            r2=cur.fetchall()
            st.write(pd.DataFrame(r2,columns=['channel_name','channel_id',
            ...
            q3= 'Select * from comments'
            cur.execute(q3)
            myconnection.commit()
            r3=cur.fetchall()
            st.write(pd.DataFrame(r3,columns=['comment_id','video_Id','comment','comment_author','comment_published']))
...
with tab3:
    ...
    view = st.checkbox('Select queries')
    if view:
        myconnection=pymysql.connect(host="127.0.0.1",user="root",password='Deva93_guvi@SQL',database='youtube_db')
        cur=myconnection.cursor()
